...
class ValueIterationAgent(ValueEstimationAgent):
        ...
        for i in range(iterations):
            ...
            for s in mdp.getStates():
                if len(mdp.getPossibleActions(s)) != 0:
                    self.values[s] = max([sum([p * (r(s,a,nexts)+discount*temp[nexts]) for nexts, p in t(s,a)]) for a in mdp.getPossibleActions(s)])
                else:
    ...
    def computeQValueFromValues(self, state, action):
        ...
        return sum([p *(r(state, action, nexts) + self.discount * self.values[nexts]) for nexts, p in t(state,action)])
