    ...
    def elapseTime(self, gameState):
        ...
        *** GORY DETAIL AHEAD ***
        ...
        for p in self.legalPositions:
            ...
            for newPos, prob in newPosDist.items():
                value = prob * self.beliefs[p]
                allPossible[newPos] += value
        allPossible.normalize()
        self.beliefs = allPossible
...
class ParticleFilter(InferenceModule):
    ...
    def initializeUniformly(self, gameState):
        ...
        quotient = self.numParticles // numOfPos
        remainder = self.numParticles % numOfPos
        for p in self.legalPositions:
            self.lofParticles.extend([p] * quotient)

        self.lofParticles.extend([random.choice(self.legalPositions) for i in range(remainder)])
    ...
    def observe(self, observation, gameState):
        ...
        "*** YOUR CODE HERE ***"
        if noisyDistance == None:
            self.lofParticles = [self.getJailPosition()] * self.numParticles
        else:
            particleWeights = util.Counter()
            for particle in self.lofParticles:
                d = util.manhattanDistance(pacmanPosition, particle)
                particleWeights[particle] += emissionModel[d]

            if particleWeights.totalCount() == 0:
                self.initializeUniformly(gameState)
            else:
                particleWeights.normalize()
                self.lofParticles = [util.sample(particleWeights) for i in range(self.numParticles)]
    ...
    def elapseTime(self, gameState):
        ...
        "*** YOUR CODE HERE ***"
        newLofParticles = []
        for par in self.lofParticles:
            newPosDist = self.getPositionDistribution(self.setGhostPosition(gameState, par))
            newLofParticles.append(util.sample(newPosDist))
        self.lofParticles = newLofParticles
...
class JointParticleFilter:
    ...
    def initializeParticles(self):
        ...
        "*** YOUR CODE HERE ***"
        self.particles = []
        possiblePer = list(itertools.product(self.legalPositions, repeat=self.numGhosts))
        quotient = self.numParticles // len(possiblePer)
        remainder = self.numParticles % len(possiblePer)
        for pos in possiblePer:
            self.particles.extend([pos] * quotient)

        self.particles.extend([random.choice(possiblePer) for i in range(remainder)])
    ...
    def observeState(self, gameState):
        ...
        ** Remember ** We store particles as tuples, but to edit a specific particle,
        ...
        for particle in self.particles:
            ...
            for i in range(self.numGhosts):
                if noisyDistances[i] != None:
                    d = util.manhattanDistance(pacmanPosition, particle[i])
                    prob *= emissionModels[i][d]
        ...
        if particleWeights.totalCount() == 0:
            ...
            particleWeights.normalize()
            self.particles = [util.sample(particleWeights) for i in range(self.numParticles)]
    ...
    def elapseTime(self, gameState):
        ...
        **Note** that you may need to replace "prevGhostPositions" with the
        ...
        for oldParticle in self.particles:
            ...
            "*** YOUR CODE HERE ***"
            positions = newParticle[:]
            for i in range(self.numGhosts):
                newPosDist = getPositionDistributionForGhost(setGhostPositions(gameState, positions), i,
                                                             self.ghostAgents[i])
